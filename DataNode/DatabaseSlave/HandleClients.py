...
def communicate(port,IP):
    print("Database slave start to listen for clients in IP:Port {}:{}".format(IP,port))
    ...
    print("Database slave start to listen for clients in IP:Port {}:{} connection stablished correctly".format(IP,port))
    
    while True:
        message = socket.recv_string()
        try:
            print("Port {} received message {}".format(port,message))
            dbcursour.execute(message)
            result= dbcursour.fetchall()
            print("Port {} received message {} replied with id={}".format(port,message,result[0][0]))
            socket.send_string("{}".format(result[0][0]))
        except:
            print("ERROR: Port {} received message {} but database error happen".format(port,message))
